...
// Static — no Shopify data or interactivity, so this stays a Server
// Component (same reasoning as TrustBadges).
//
// Path data is the official monochrome mark from Simple Icons (MIT
// licensed path data — https://simpleicons.org; trademark rights in the
// underlying logos belong to their respective owners, used here under
// ordinary nominative fair use to indicate accepted payment methods, the
// same way every ecommerce checkout does). Each viewBox is cropped to
// that icon's actual drawn bounding box (verified via a real browser's
// SVGGeometryElement.getBBox(), not eyeballed) rather than Simple Icons'
// uniform 24x24 canvas — several of these are wordmarks far wider than
// tall, and sizing them all from an uncropped square viewBox would leave
// them inconsistently padded and visually mismatched against each other.
type IconProps = ComponentProps<"svg"> & { label: string };

function PaymentIcon({ label, children, ...props }: IconProps) {
  return (
    <svg role="img" aria-label={label} fill="currentColor" {...props}>
      {children}
    </svg>
  );
}

function IconVisa(props: ComponentProps<"svg">) {
  return (
    <PaymentIcon label="Visa" viewBox="-0.5 7.62 25 8.75" {...props}>
...
function IconAmex(props: ComponentProps<"svg">) {
  return (
    <PaymentIcon label="American Express" viewBox="0 0 24 24" {...props}>
...
function IconApplePay(props: ComponentProps<"svg">) {
  return (
    <PaymentIcon label="Apple Pay" viewBox="-0.5 3.82 25 16.37" {...props}>
...
function IconGooglePay(props: ComponentProps<"svg">) {
  return (
    <PaymentIcon label="Google Pay" viewBox="-0.5 6.73 25 10.53" {...props}>
...
function IconPayPal(props: ComponentProps<"svg">) {
  return (
    <PaymentIcon label="PayPal" viewBox="1.32 -0.5 21.36 25" {...props}>
